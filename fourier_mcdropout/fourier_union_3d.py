...
def sum_std_from_tensors(std_list_a, std_list_b):
    ...
    return sum_std
class aro_3d(nn.Module):
    def __init__(self, fno_low,fno_res):
        super(aro_3d, self).__init__()
        modes1_low, modes2_low, modes3_low,width_low = fno_low
        modes1_res, modes2_res, modes3_res,width_res = fno_res
    ...
    def training_low(self,data_low,settings):
        ...
        model = FNO3d(self.modes1_low, self.modes2_low,self.modes3_low, self.width_low).cuda()
    ...
    def training_res(self,data_res,settings):
        ...
        ntrain_low, ntrain_high,ep,bsize,seed= settings
        model = FNO3d(self.modes1_res, self.modes2_res, self.modes3_res,self.width_res).cuda()
    ...
    def test_model(self,x_test,y_test,model_low,model_res,x_normalizer_low,y_normalizer_low,x_normalizer_high,y_normalizer_high):
        ...
        print("r2:", total_r2)
        print("max.Terr", total_maxx)
        print("mean.Terr", total_differ)
        print("mape", total_mape)
        print("pape", total_pape)
        t2 = default_timer()
        # print("each time:", (t2 - t1) / x_test.shape[0])
